    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <h3 class="mb-0 text-culture-green">
                <i class="bi bi-plus-circle-fill me-2"></i>Ajouter un Commentaire
            </h3>
        </div>
        <div class="col-sm-6">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header card-header-culture">
                <h3 class="card-title mb-0 text-white">
                    <i class="bi bi-chat-square-text me-2"></i>Détails du Commentaire
                </h3>
            </div>
                ...
                <form action="{{ route('admin.commentaires.store') }}" method="POST">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select class="form-select @error('id_utilisateur') is-invalid @enderror" id="id_utilisateur" name="id_utilisateur" required>
                                ...
                                @foreach($utilisateurs as $id => $nom)
                                    <option value="{{ $id }}" {{ old('id_utilisateur') == $id ? 'selected' : '' }}>{{ $nom }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select class="form-select @error('id_contenu') is-invalid @enderror" id="id_contenu" name="id_contenu" required>
                                ...
                                @foreach($contenus as $id => $titre)
                                    <option value="{{ $id }}" {{ old('id_contenu') == $id ? 'selected' : '' }}>{{ Str::limit($titre, 40) }}</option>
                                @endforeach
                            </select>
                            @error('id_contenu')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="number" class="form-control @error('note') is-invalid @enderror" id="note" name="note" value="{{ old('note') }}" min="1" max="5">
                            @error('note')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <small class="text-muted">La date de création sera utilisée si ce champ est vide.</small>
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('texte') is-invalid @enderror" id="texte" name="texte" rows="5" required>{{ old('texte') }}</textarea>
                        @error('texte')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
